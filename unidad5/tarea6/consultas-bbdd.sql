...
sqlite> select c.* from clientes as c, pedidos as p where c.id = p.id_cliente and p.fecha_pedido regexp '2024-02-' group by c.id having count(p.id_cliente)>1;
...
select * from productos where precio in (select max(precio))
